...
/**
 * All citty command definitions (kept out of index.ts for maintainability).
 */
...
const init = defineCommand({
  meta: {
    name: "init",
    description: "Initialize SDD + ONE AI agent. Example: sdd init --here --ai grok",
  },
  args: {
    path: {
      type: "positional",
...
const newCmd = defineCommand({
  meta: {
    name: "new",
    description: "Start a change pack, refresh handoff, and launch the AI agent from init",
  },
  args: {
  ...
  async run({ args }) {
    await withProject(async ({ root, config }) => {
      ...
      const firstArt = stage?.artifacts.find((a) => a.required) ?? stage?.artifacts[0];
      if (firstArt) {
        ...
        consola.log(
          `  3. Run ${pc.cyan("sdd next")}  (or ${pc.cyan("sdd agent")} to draft with AI)`,
        );
        consola.log("");
        ...
        consola.log(
          pc.dim("  Success: open expenses with zero rows — no error, empty state visible."),
        );
        consola.log("");
      }
...
const skip = defineCommand({
  ...
  args: {
    ...
    reason: {
      type: "string",
      description: "Why this stage is skipped",
      alias: "r",
      required: true,
    },
    change: { type: "string", description: "Change id", alias: "c" },
...
const featureStart = defineCommand({
  ...
  async run({ args }) {
    await withProject(async ({ root, config }) => {
      ...
      consola.success(`Started ${pc.cyan(feature.id)} → change ${pc.cyan(ctx.id)}`);
      ...
      consola.log("");
      consola.log(
        pc.dim("Backlog status set to in_progress. When done: sdd verify → sdd complete"),
      );
      consola.log("");
...
const contextCmd = defineCommand({
  ...
  args: {
    ...
    out: {
      type: "string",
      description: "Write regenerable markdown path, or 'change' for changes/<id>/code-context.md",
      alias: "o",
    },
    stdout: {
      type: "boolean",
      ...
      try {
        ...
        if (args.out) {
          if (args.out === "change") {
            ...
            writeArtifactTo = join(changePath(root, config, changeId), "code-context.md");
          } else {
            writeArtifactTo = args.out.startsWith("/") ? args.out : join(root, args.out);
          }
        }
        ...
        const wantStdout = args.stdout || (!args.out && !args.json) || (args.out && args.stdout);

        if (args.json) {
          process.stdout.write(
            formatJsonSummary(result.summary, result.gaps, result.slices, result.ok) + "\n",
          );
        } else if (wantStdout) {
          process.stdout.write(
          ...
          consola.success(`Wrote code context → ${pc.cyan(result.artifactPath)}`);
          consola.log(
            pc.dim(
...
const refine = defineCommand({
  ...
  async run({ args }) {
    await withProject(async ({ root, config }) => {
      ...
      consola.log(
        pc.dim(`Focus artifacts: ${plan.focusArtifacts.map((a) => a.path).join(", ") || "(none)"}`),
      );
      consola.log(
        pc.dim(
...
const doctor = defineCommand({
  ...
  async run() {
    ...
    line(
      "SDD init",
      initialized,
      initialized ? ".sdd/config.yaml found" : "run: sdd init --here --ai copilot",
    );
...
const help = defineCommand({
  ...
  async run() {
    ...
    consola.log(
      `  ${pc.cyan("sdd mcp sources add …")}  attach design-system / AST / org MCP for sdd to call`,
    );
    consola.log(
      `  ${pc.cyan("sdd mcp fetch")}            pull matching MCP context for current stage`,
    );
    consola.log(
      pc.dim(
        "  If next fails: open the file in the error and write real sentences (not empty template).",
      ),
    );
    consola.log("");
    consola.log(
      pc.dim("Docs: https://harsha09.github.io/spec-driven-development/tutorials/first-change/"),
    );
    consola.log(
      pc.dim(
